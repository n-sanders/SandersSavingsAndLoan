(async function () {
  const { api, requireUser } = SslApi;
  const me = await requireUser();
  if (!me) return;

  const homeLink = document.getElementById("home-link");
  homeLink.href = me.role === "Banker" ? "/banker.html" : "/kid.html";
  homeLink.textContent = me.role === "Banker" ? "Banker desk" : "My savings";
  ...
  function renderPresets() {
    ...
    for (const [name, colors] of Object.entries(SslThemes.themes)) {
      ...
      btn.innerHTML = `
        ...
        <div class="theme-preset-name">${SslThemes.displayName(name)}</div>
      `;
      btn.addEventListener("click", () => {
        state = SslTheme.setPreset(name);
        draftColors = SslThemes.cloneColors(state.colors);
        syncColorInputs();
        renderPresets();
      });
  ...
  function updateCustomColor(key, rawValue) {
    ...
    if (!hex) return;
    draftColors[key] = hex;
    state = SslTheme.setCustom(draftColors);
    syncColorInputs();
    renderPresets();
  }
  ...
  function renderColorEditor() {
    ...
    for (const key of SslThemes.COLOR_KEYS) {
      ...
      field.innerHTML = `
        <label for="color-${key}">${SslThemes.COLOR_LABELS[key]}</label>
